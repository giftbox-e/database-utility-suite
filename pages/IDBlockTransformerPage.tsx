...
const IDBlockTransformerPage: React.FC = () => {
    ...
    }, [inputText, keywordsText, blockStartFormat, applyToBlock, sourceKey, condition, conditionValue, targetKey, operation, operationValue, roundDecimals]);
    ...
    return (
        <div className="bg-gray-800 rounded-lg shadow-2xl p-4 sm:p-6 border border-gray-700 flex flex-col h-full">
            <div className="flex-shrink-0 mb-6 p-3 sm:p-4 border border-gray-700 rounded-lg bg-gray-800 overflow-y-auto max-h-[50vh]">
                <div className="flex items-center space-x-2 mb-3">
                    <h2 className="text-lg font-semibold text-white">ID Block Transformer</h2>
                ...
                <details className="group bg-gray-900/40 border border-gray-700/50 rounded-lg mb-4">
                    <summary className="flex cursor-pointer items-center justify-between p-3 text-sm font-medium text-gray-300 hover:text-white transition-colors">
                        <span>Isolate blocks by their IDs to calculate target values. <span className="text-indigo-400">See more...</span></span>
                        ...
                        <div className="space-y-4 p-3 rounded-md border border-gray-700">
                             <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="e.g.&#10;22526&#10;22527"
                                    className="w-full bg-gray-900 text-gray-300 placeholder:text-gray-500 border border-gray-600 rounded-md shadow-sm p-2 font-mono text-sm focus:ring-2 focus:ring-indigo-500 h-24"
                                />
                            </div>
                            <div>
                                <div className="flex items-center space-x-2">
                                    <label htmlFor="block-format" className="block text-sm font-medium text-gray-300 mb-1">Block Start Format</label>
            ...
            <div className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0">
                <div className="flex flex-col min-h-0">
                    <div className="flex-shrink-0 flex items-center justify-between mb-2">
                        ...
                        <textarea 
                            ...
                            className={`flex-grow w-full bg-gray-900 text-gray-300 placeholder:text-gray-500 border rounded-md shadow-sm p-4 font-mono text-sm focus:ring-2 focus:ring-indigo-500 transition-all ${isDragging ? 'border-blue-500 ring-2 ring-blue-500' : 'border-gray-600'}`} 
                            spellCheck="false"
                ...
                <div className="flex flex-col min-h-0">
                    ...
                    <textarea id="output-text" value={outputText} readOnly placeholder="Result will appear here after processing..." className="flex-grow w-full bg-gray-900 text-gray-300 placeholder:text-gray-500 border border-gray-600 rounded-md shadow-sm p-4 font-mono text-sm" spellCheck="false" />
                </div>
            </div>
            
            <div className="flex-shrink-0 mt-6 flex flex-col sm:flex-row items-center justify-center gap-4">
                <button onClick={handleProcess} disabled={isProcessButtonDisabled} className="w-full sm:w-auto flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-indigo-500 disabled:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                    {isProcessing ? <><LoadingSpinner/>Processing...</> : <><ProcessIcon/>Transform Data</>}
                </button>
                <button onClick={handleCopy} disabled={!outputText} className="w-full sm:w-auto flex items-center justify-center px-6 py-3 border border-gray-600 text-base font-medium rounded-md shadow-sm text-gray-200 bg-gray-700 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                     <CopyIcon />{copyStatus}
                </button>
                <button onClick={handleDownload} disabled={!outputText} className="w-full sm:w-auto flex items-center justify-center px-6 py-3 border border-gray-600 text-base font-medium rounded-md shadow-sm text-gray-200 bg-gray-700 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
